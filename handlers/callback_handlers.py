...
async def select_all_questions(user_id):
    cur.execute(
        f'CREATE TABLE user_{user_id}(id INTEGER, question TEXT, answer_1 TEXT, answer_2 TEXT, answer_3 TEXT, answer_4 TEXT, correct_answer INTEGER, explanation TEXT)'
    )
    conn.commit()
    ...
    conn.commit()
    cur.execute(
        f'CREATE TABLE quiz_{user_id} AS SELECT * FROM user_{user_id} LIMIT 7')
    conn.commit()
...
async def start_quiz(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.answer_poll(
            ...
            options=[questions[0][2], questions[0][3],
                     questions[0][4], questions[0][5]],
            type='quiz',
        ...
        cur.execute(
            f'CREATE TABLE IF NOT EXISTS quiz_{user_id} AS SELECT * FROM user_{user_id} LIMIT 7')
        conn.commit()
        ...
        if len(questions) == 7:
            ...
            await call.message.answer_poll(
                ...
                options=[questions[0][2], questions[0][3],  # type: ignore
                         questions[0][4], questions[0][5]],  # type: ignore
                type='quiz',
...
async def rules(call: CallbackQuery):
    ...
    rules_btn = InlineKeyboardButton(
        text='ПРОЧИТАТЬ ЕЩЁ РАЗ', callback_data='rules')
    inkb = InlineKeyboardMarkup(row_width=1).add(start_btn).add(rules_btn)
